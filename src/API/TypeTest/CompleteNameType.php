<?php

namespace jamesiarmes\PEWS\API\TypeTest;

use jamesiarmes\PEWS\API\TypeTest;

/**
 * Class representing CompleteNameType
 *
 *
 * XSD Type: CompleteNameType
 *
 * @method CompleteNameType getTitle()
 * @method CompleteNameType setTitle($title)
 * @method CompleteNameType getFirstName()
 * @method CompleteNameType setFirstName($firstName)
 * @method CompleteNameType getMiddleName()
 * @method CompleteNameType setMiddleName($middleName)
 * @method CompleteNameType getLastName()
 * @method CompleteNameType setLastName($lastName)
 * @method CompleteNameType getSuffix()
 * @method CompleteNameType setSuffix($suffix)
 * @method CompleteNameType getInitials()
 * @method CompleteNameType setInitials($initials)
 * @method CompleteNameType getFullName()
 * @method CompleteNameType setFullName($fullName)
 * @method CompleteNameType getNickname()
 * @method CompleteNameType setNickname($nickname)
 * @method CompleteNameType getYomiFirstName()
 * @method CompleteNameType setYomiFirstName($yomiFirstName)
 * @method CompleteNameType getYomiLastName()
 * @method CompleteNameType setYomiLastName($yomiLastName)
 */
class CompleteNameType extends TypeTest
{

    /**
     * @property string $title
     */
    protected $title = null;

    /**
     * @property string $firstName
     */
    protected $firstName = null;

    /**
     * @property string $middleName
     */
    protected $middleName = null;

    /**
     * @property string $lastName
     */
    protected $lastName = null;

    /**
     * @property string $suffix
     */
    protected $suffix = null;

    /**
     * @property string $initials
     */
    protected $initials = null;

    /**
     * @property string $fullName
     */
    protected $fullName = null;

    /**
     * @property string $nickname
     */
    protected $nickname = null;

    /**
     * @property string $yomiFirstName
     */
    protected $yomiFirstName = null;

    /**
     * @property string $yomiLastName
     */
    protected $yomiLastName = null;


}

